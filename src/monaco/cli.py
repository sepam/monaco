...
@click.option('--name', '-n', default='My Project',
              help='Project name for the template')
...
def run(config_file: str, simulations: int, output: Optional[str],
        output_format: str, seed: Optional[int]) -> None:
    ...
    try:
        ...
        if output:
            project.export_results(n=simulations, format=output_format,
                                   output=output)
...
@click.option('--no-durations', is_flag=True,
              help='Hide duration information on nodes')
def graph(config_file: str, output: Optional[str], no_durations: bool) -> None:
    ...
    try:
        ...
        project.plot_dependency_graph(
            save_path=output,
            show_durations=not no_durations
        )
